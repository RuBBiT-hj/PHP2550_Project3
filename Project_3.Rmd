...
The prediction model for CVD risk is based on previous studies [@heart]. We fit the logistic regression model on the Framingham train set for each sex and apply to the test set.

### Transportability Analysis

To avaluate the prediction performance on the target population NHANES, we use the brier score formula below [@Steingrimsson2023-og]:
...
# the imputation is done within train set
## and apply the same model to the test
...
for(i in 1:m){
  imp.train.data.men <- complete(imp.train.men, i)
  imp.train.data.women <- complete(imp.train.women, i)
  
  # create the variables to match framingham
  ## for men
  imp.train.data.men$SYSBP_UT <- ifelse(imp.train.data.men$BPMEDS == 0,
                                  imp.train.data.men$SYSBP, 0)
  imp.train.data.men$SYSBP_T <- ifelse(imp.train.data.men$BPMEDS == 1,
                                  imp.train.data.men$SYSBP, 0)
  imp.train.data.men$CVD <- NA
  imp.train.data.men$SEX <- 1
  imp.train.data.men <- imp.train.data.men %>% 
    select(colnames(train_framingham_men))
    
  ## for women
  imp.train.data.women$SYSBP_UT <- ifelse(imp.train.data.women$BPMEDS == 0,
                                  imp.train.data.women$SYSBP, 0)
  imp.train.data.women$SYSBP_T <- ifelse(imp.train.data.women$BPMEDS == 1,
                                  imp.train.data.women$SYSBP, 0)
  imp.train.data.women$CVD <- NA
  imp.train.data.women$SEX <- 2
  imp.train.data.women <- imp.train.data.women %>% 
    select(colnames(train_framingham_women))
  
  imp.test.data.men <- complete(imp.test.men, i)
  imp.test.data.women <- complete(imp.test.women, i)
  
  ## for men, test
  imp.test.data.men$SYSBP_UT <- ifelse(imp.test.data.men$BPMEDS == 0,
                                  imp.test.data.men$SYSBP, 0)
  imp.test.data.men$SYSBP_T <- ifelse(imp.test.data.men$BPMEDS == 1,
                                  imp.test.data.men$SYSBP, 0)
  imp.test.data.men$CVD <- NA
  imp.test.data.men$SEX <- 1
  imp.test.data.men <- imp.test.data.men %>% 
    select(colnames(test_framingham_men))
    
  ## for women, test
  imp.test.data.women$SYSBP_UT <- ifelse(imp.test.data.women$BPMEDS == 0,
                                  imp.test.data.women$SYSBP, 0)
  imp.test.data.women$SYSBP_T <- ifelse(imp.test.data.women$BPMEDS == 1,
                                  imp.test.data.women$SYSBP, 0)
  imp.test.data.women$CVD <- NA
  imp.test.data.women$SEX <- 2
  imp.test.data.women <- imp.test.data.women %>% 
    select(colnames(test_framingham_women))
  
  # match the variables
  full.train.men <- data.frame(rbind(imp.train.data.men, train_framingham_men))
  full.train.women <- data.frame(rbind(imp.train.data.women, train_framingham_women))
  
  full.test.men <- data.frame(rbind(imp.test.data.men, test_framingham_men))
  full.test.women <- data.frame(rbind(imp.test.data.women, test_framingham_women))
  
  # add the population indicator - 1 for framingham and 0 for nhanes
  full.train.men$S <- c(rep(0, nrow(imp.train.data.men)), 
                        rep(1, nrow(train_framingham_men)))
  full.train.women$S <- c(rep(0, nrow(imp.train.data.women)), 
                          rep(1, nrow(train_framingham_women)))
  full.test.men$S <- c(rep(0, nrow(imp.test.data.men)), 
                        rep(1, nrow(test_framingham_men)))
  full.test.women$S <- c(rep(0, nrow(imp.test.data.women)), 
...
for (i in 1:1000) {
  ...
  full.sim.test.men <- rbind(MN_sample_men[index_test_nhanes_men, ],
                           test_framingham_men[, 3:12])
  full.sim.test.men$S <- c(rep(0, length(index_test_nhanes_men)),
                           rep(1, nrow(test_framingham_men)))
  ...
  # form the new test set
  full.sim.test.women <- rbind(MN_sample_women[index_test_nhanes_women, ],
                           test_framingham_women[, 3:12])
  full.sim.test.women$S <- c(rep(0, length(index_test_nhanes_women)),
                           rep(1, nrow(test_framingham_women)))
...
for (i in 1:1000) {
  ...
  # form the new test set
  full.sim.test.men <- rbind(bootstrap_sample_men[index_test_nhanes_men, ],
                           test_framingham_men[, 3:12])
  full.sim.test.men$S <- c(rep(0, length(index_test_nhanes_men)),
                           rep(1, nrow(test_framingham_men)))
  ...
  # form the new test set
  full.sim.test.women <- rbind(bootstrap_sample_women[index_test_nhanes_women, ],
                           test_framingham_women[, 3:12])
  full.sim.test.women$S <- c(rep(0, length(index_test_nhanes_women)),
                           rep(1, nrow(test_framingham_women)))
...
df_performance[3, 1] <- paste0(
  round(mean( (MN_BS_men - mean(brier_nhanes_men) )^2), 5), " (",
  round(sqrt(var( (MN_BS_men - mean(brier_nhanes_men) )^2)/1000), 5), ")")
